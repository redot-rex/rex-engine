...
// just a fair warning, this code is extremely architectually, compiler, and operating system dependent
...
FORCE_INLINE uint32_t check_mask(uint32_t value, uint32_t n)
{	
	return (value & (1 << n)) != 0;
}
...
// to understand, the gnu version also checks if cpuid exists... since in msvc we cannot compile 32 bit it doesn't matter and we always will return 1
FORCE_INLINE int cpuid(int leaf, uint32_t* a, uint32_t* b, uint32_t* c, uint32_t* d )
{
	#if defined(__GNUC__) || defined(__clang__)
...
FORCE_INLINE int cpuidex(int leaf, int subleaf, uint32_t* a, uint32_t* b, uint32_t* c, uint32_t* d)
{
	#if defined(__GNUC__) || defined(__clang__)
...
FORCE_INLINE uint32_t intrin_support()
{
	
	uint32_t intrin_support = 0;
	uint32_t a, b, c, d;

	// we first check for intrinsics support for avx instructions as these are the most compiler dependent due to the xcr checks
	cpuid(1, &a, &b, &c, &d);
	...
	if (avxcheck && osxsave)
	{
		...
		intrin_support |= (check_mask(c, 28) << 7) && avxcheck; // avx1
	}
	...
	if (cpuidex(7, 0, &a, &b, &c, &d) && avxcheck) 
	{
		...
		intrin_support |= check_mask(b, 16) << 9;   // avx512f all below will be represented by their suffix
...
[[nodiscard]]
FORCE_INLINE intrin_check intrin_compat()
{
